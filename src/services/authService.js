...
const login = async (email, password) => {
  ...
  if (count >= MAX_ATTEMPTS()) {
    logger.warn('Login blocked — max attempts reached', { email: normalizedEmail });
    throw new AppError(
      `Account locked. Too many failed attempts. Try again in ${Math.ceil(ttl / 60)} minute(s).`,
      423
    );
  }
  ...
  if (!user || !isMatch) {
    ...
    logger.warn('Login failed — invalid credentials', { email: normalizedEmail, attempts });
    throw new AppError(
      remaining > 0
        ? `Invalid email or password. ${remaining} attempt(s) remaining.`
        : `Account locked. Try again in ${Math.ceil(BLOCK_DURATION() / 60)} minute(s).`,
      401
    );
  }
  ...
  await setSession(
    sessionId,
    { userId: user._id.toString(), fullName: user.fullName, email: user.email, loginTime: new Date().toISOString() },
    SESSION_EXPIRY()
  );
...
const getSessionInfo = (sessionId) => getSession(sessionId);
